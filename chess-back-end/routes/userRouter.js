...
const { CLIENT_URL, JWT_SECRET, GMAIL_USER } = require('../config/config');
...
Router.post('/register', async (req, res) => {
    ...
    try {
        ...
        // send email verification mail to user and to self
        jwt.sign({ user: user._id }, JWT_SECRET, { expiresIn: '1d' }, (err, emailToken) => {
            if (err) {
                console.log(err);
            }
            const url = `${CLIENT_URL}/u/confirmation/${emailToken}`; // need to fix this to work with client url and also on the clietn
            transporter
                .sendMail({
                    to: user.email,
                    subject: 'Account confirmation for Chess ',
                    html: `<h3>You registered for Chess.</h3>Please click on this <a href="${url}">link</a> to confirm your email`,
                })
                .catch((error) => {
                    console.log(error);
                ...
                .sendMail({
                    to: GMAIL_USER,
                    subject: 'Account confirmation for Chess-Copy ',
                    html: `<h3>You registered for Chess.</h3>Please click on this <a href="${url}">link</a> to confirm your email`,
                })
                .catch((error) => {
                    console.log(error);
...
Router.get('/confirmation/:token', async (req, res) => {
    try {
        ...
        const id = data.user;
        const redirectUrl=`${CLIENT_URL}/login?EmailConfirmedRedirect=true`
...
Router.post('/resetPassword', async (req, res) => {
    try {
        ...
        transporter
            .sendMail({
                to: user.email,
                subject: 'Password reset for Chess',
                html: `
...
Router.get('/:userId/rating', userAuth, async (req, res) => {
    try {
        // get user data for rating page
        let user = await User.findById(req.query.userId);
        if (user) {
            res.status(200).json({ username: user.username });
        } else {
