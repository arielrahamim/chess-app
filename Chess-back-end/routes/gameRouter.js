...
Router.post('/gen-random', async (req, res) => {
    try {
        // create a game with given fen position
        let game = new Game({
            fen: req.body.fen,
        ...
        game = await game.save();
        res.json({ gameId: game._id , rand: game.Regular});
    } catch (error) {
        console.log(error);
        return res.status(500).json({ error: 'server error' });
    }
...
Router.post('/join-random', async (req, res) => {
    try {
        const game = await Game.findOne({ Regular: false, bothPlayerJoined: false });
        if (game) {
            res.json({ gameId: game._id, rand: game.Regular });
        } else {
            res.status(404).send('/gen-random');
        }
    } catch (error) {
        console.log(error);
        if (error.response && (error.response.status === 400 || error.response.status === 500 || error.response.status === 403 || error.response.status === 404)) {
            res.status(404).send('/gen-random');
        } else {
